...
parser.add_argument('rough_in', help='Name of roughness data file w/o extension')
...
def MetaData(topo_in,bound_in,rough_in,res,t_to_print,duration,inlet_btag,outlet_btag,q,slope):
    #Create list of parameters for metadata file for each model result
    Parameter_names = ["topo_in:","bound_in:","rough_in:","res:","t_to_print:","duration:","inlet_btag:","outlet_btag:","q:","slope:"]
    Parameter_vals = [topo_in,bound_in,rough_in,res,t_to_print,duration,inlet_btag,outlet_btag,q,slope]
    Parameters = ["","","","","","","","","",""]
    for i in numpy.arange(0,10):
        Parameters[i] = str(Parameter_names[i])+" "+str(Parameter_vals[i])

    #Save Parameter List as Metadata
    parlist = topo_in+'_'+str(q)+'cms_'+str(res)+'m_'+rough_in
...
def SetQuant(topo_in,rough_in,domain):
    ...
    domain.set_quantity('friction', filename=rough_in+'.pts')
...
def Main(topo_in,bound_in,rough_in,datadir,res,t_to_print,duration,inlet_btag,outlet_btag,q,slope):
    parlist = MetaData(topo_in,bound_in,rough_in,res,t_to_print,duration,inlet_btag,outlet_btag,q,slope)
    res = int(res)
    ...
    SetRoughness(rough_in)
    domain = SetQuant(topo_in,rough_in,domain)
    domain = SetBC(q,slope,domain)
    print('Running Simulation')
    Evolve(t_to_print,duration,domain)

Main(args.topo_in,args.bound_in,args.rough_in,args.datadir,args.res,args.t_to_print,args.duration,args.inlet_btag,args.outlet_btag,args.q,args.slope)
